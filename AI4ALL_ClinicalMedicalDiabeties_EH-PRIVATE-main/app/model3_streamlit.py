    ...
    key_panel(
        {
            ...
            "Macro F1": "Class-balanced score comparing performance across both outcome groups.",
...
metric_cols[1].metric("Macro F1", f"{metrics['f1_macro']:.3f}")
...
with gauge_cols[1]:
    ...
    visual_notes(
        ...
        [
            "This score checks whether the model is balanced.",
...
left_static, right_static = st.columns(2, gap="large")
with left_static:
...
st.markdown(
    """
    Model 3 is the ensemble random-forest model. The feature-importance results
    summarizes which selected predictors contributed most to the fitted forest.
    """
)
