...
def st_echarts(options: dict, height: str, key: str, **kwargs):
    ...
    st.warning(
        "Interactive ECharts could not load in this environment. "
        "Showing the chart configuration fallback so the app can continue."
    )
    if _ECHARTS_IMPORT_ERROR is not None:
        st.caption(f"ECharts import issue: {_ECHARTS_IMPORT_ERROR.__class__.__name__}")
...
def visual_notes(title: str, technical: list[str], takeaways: list[str]) -> None:
...
def metric_bar_chart(results: pd.DataFrame, key: str) -> None:
    ...
    options = {
        ...
        "grid": {"left": 36, "right": 18, "top": 22, "bottom": 58},
...
def confusion_heatmap(matrix: list[list[int]], key: str) -> None:
    ...
    options = {
        ...
        "grid": {"left": 88, "right": 34, "top": 32, "bottom": 54},
...
def horizontal_importance_chart(df: pd.DataFrame, key: str, limit: int = 12) -> None:
    ...
    options = {
        "tooltip": {"trigger": "axis", "axisPointer": {"type": "shadow"}},
        "grid": {"left": 170, "right": 28, "top": 20, "bottom": 28},
...
def stacked_outcome_bar(matrix: list[list[int]], key: str) -> None:
    ...
    options = {
        ...
        "grid": {"left": 130, "right": 28, "top": 48, "bottom": 22},
        "xAxis": {"type": "value", **_axis_common()},
...
def mixed_metric_combo(rows: pd.DataFrame, key: str) -> None:
    ...
    options = {
        ...
        "legend": {"top": 0, "textStyle": {"color": TEXT_COLOR, "fontSize": TEXT_SIZE}},
        "grid": {"left": 38, "right": 20, "top": 50, "bottom": 42},
